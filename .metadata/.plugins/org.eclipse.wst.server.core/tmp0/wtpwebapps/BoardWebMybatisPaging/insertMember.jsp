	...
	<form action="insertMember.do" method="post" name="insertMemberForm">
		<table>
			<tr>
				<th width="20%">아이디</th>
				<td width="80%">
					<c:if test="${inId != 'noId' }"><input type="text" name="id" class="id" value="${inId }"></c:if>
					<c:if test="${inId == 'noId' }"><input type="text" name="id" class="id"></c:if>
					<input type="button" value="아이디 중복 체크" id="btn_id_check"><br>
					<c:if test="${inId != 'noId' }"><div class="ss s1">사용 가능한 아이디</div></c:if>
					<c:if test="${inId == 'noId' }"><div class="ss s2">아이디 중복! 사용 불가</div></c:if>
				</td>
			</tr>
			<tr>
				<th>비밀번호</th>
